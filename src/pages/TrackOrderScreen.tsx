...
export const TrackOrderScreen: React.FC = () => {
  const { currentOrder, setActiveTab, userProfile } = useApp();
  ...
  const orderId = currentOrder?.id ? `#CC-${currentOrder.id.replace(/^QM-/, '').slice(0, 6).toUpperCase()}` : '#CC-928410';
  const deliveryAddressText = currentOrder?.deliveryAddress || userProfile.address || 'Jaydev Vihar, Bhubaneswar';
  const driverName = currentOrder?.driverName || 'Bikash Mohanty';
  const driverPhone = currentOrder?.driverPhone || '+91 98000 11111';
  const driverRating = currentOrder?.driverRating || 4.9;
  const etaMinutes = currentOrder?.estimatedDeliveryMinutes || 12;

  const handleSupportWhatsApp = () => {
    const url = `https://wa.me/919800011111?text=${encodeURIComponent(`Hi CartCraze Support, I need help with my active order ${orderId}`)}`;
    ...
    <div className="flex flex-col relative w-full pb-24 font-sans animate-fadeIn">
      ...
      <div className="relative w-full h-[380px] overflow-hidden bg-[#e9f0f2]">
        ...
        >
          ...
          {/* Hyperlocal Labels */}
          <text fill="#6e797a" fontFamily="Inter" fontSize="8" fontWeight="600" letterSpacing="1" transform="rotate(-62 145 270)" x="145" y="270">JAYDEV VIHAR - NANDANKANAN RD</text>
          <text fill="#6e797a" fontFamily="Inter" fontSize="8" fontWeight="600" letterSpacing="0.8" transform="rotate(-15 210 105)" x="210" y="105">IRC VILLAGE MAIN RD</text>
          <text fill="#00828a" fontFamily="Inter" fontSize="8" fontWeight="700" opacity="0.7" x="12" y="150">EKAMRA KANAN LAKE</text>
        </svg>
        ...
        <div className="absolute top-2 inset-x-0 px-3 flex items-center justify-between z-20">
          <button 
            ...
            className="w-9 h-9 rounded-full bg-white shadow-md flex items-center justify-center text-[#131b2e] active:scale-95 transition-transform cursor-pointer"
          >
          ...
          <button 
            ...
            className="w-9 h-9 rounded-full bg-white shadow-md flex items-center justify-center text-[#00676d] active:scale-95 transition-transform cursor-pointer"
          >
        ...
        <div className="absolute top-14 inset-x-0 flex justify-center px-4 z-20 pointer-events-none">
          <div className="pointer-events-auto flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#131b2e] shadow-xl text-white animate-bounce" style={{ animationDuration: '3s' }}>
            ...
            <span className="text-xs font-extrabold text-white tracking-wide">Delivering in {etaMinutes} Mins</span>
            <span className="w-1 h-1 rounded-full bg-white/40"></span>
            <span className="text-[10px] text-white/90">Express Delivery</span>
          </div>
        </div>

        {/* Origin Marker (Store #01 Jaydev Vihar) */}
        ...
        <div className="absolute top-[82px] left-[322px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-10">
          ...
          <div className="mt-0.5 px-2 py-0.2 rounded-full bg-white text-[#131b2e] text-[9px] font-bold shadow-md whitespace-nowrap max-w-[120px] truncate">
            {deliveryAddressText}
          </div>
        </div>
        ...
        <div className="absolute bottom-5 right-3 z-10">
          <button 
            ...
            className="w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center text-[#131b2e] active:scale-90 transition-transform cursor-pointer"
          >
      ...
      <div className="relative -mt-4 w-full bg-white rounded-t-3xl shadow-[0_-8px_24px_rgba(19,27,46,0.08)] px-4 pt-2 pb-8 z-30 flex flex-col gap-3.5 border-t border-[#eaedff]">
        ...
        <div className="flex items-start justify-between gap-2 pt-0.5">
          <div className="flex flex-col">
            ...
            <p className="text-xs text-[#6e797a] mt-0.5">
              {driverName} is delivering from CartCraze Jaydev Vihar on an EV scooter
            </p>
          </div>
        ...
        <div className="w-full py-2 px-2 bg-[#f2f3ff] rounded-2xl">
          <div className="grid grid-cols-4 relative items-center gap-0">
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              ...
              <span className="text-[8px] text-[#6e797a]">Confirmed</span>
            </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              ...
              <span className="text-[8px] text-[#6e797a]">Store #01</span>
            </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              ...
              <span className="text-[8px] font-bold text-[#006a48]">Live</span>
            </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10 opacity-50">
              ...
              <span className="text-[8px] text-[#6e797a]">~{etaMinutes} min</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-2.5 rounded-2xl bg-[#f2f3ff] gap-2 border border-[#eaedff]">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-1">
                <h3 className="text-xs font-bold text-[#131b2e] truncate">{driverName}</h3>
              ...
              <p className="text-[10px] text-[#6e797a] truncate mt-0.5">1,400+ deliveries • Jaydev Vihar Fleet</p>
              <div className="flex items-center gap-1 mt-0.5">
                <span className="material-symbols-outlined text-[11px] text-[#006a48]">health_and_safety</span>
                <span className="text-[9px] text-[#006a48] font-bold tracking-tight">Thermal Monitored &amp; Masked</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button 
              onClick={handleSupportWhatsApp}
              className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#00676d] hover:bg-[#eaedff] active:scale-95 transition-all shadow-2xs cursor-pointer"
              title="Chat with Delivery Partner via WhatsApp"
            >
              <span className="material-symbols-outlined text-[18px]">chat</span>
            </button>
            <a 
              href={`tel:${driverPhone}`}
              className="w-9 h-9 rounded-full bg-[#00676d] flex items-center justify-center text-white hover:bg-[#00828a] active:scale-95 transition-all shadow-sm cursor-pointer"
        ...
        <div className="p-2.5 rounded-2xl bg-[#f2f3ff] flex flex-col gap-1.5 border border-[#eaedff]">
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-1.5">
              ...
              <div className="flex flex-col">
                <span className="text-xs font-bold text-[#131b2e]">Delivering to</span>
        ...
        >
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-bold text-[#131b2e] truncate">
                  {currentOrder?.items?.length ? `${currentOrder.items.length} Items in this bag` : 'Items in this bag'}
                </span>
                <span className="text-[10px] text-[#6e797a] truncate">
                  {currentOrder?.items?.length 
                    ? currentOrder.items.map((i) => i.product.name).join(', ')
                    : 'CartCraze Fresh Grocery Items'}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <span className="text-xs font-extrabold text-[#131b2e]">
                ₹{currentOrder?.finalTotal || 373.50}
              </span>
              ...
              {currentOrder?.items && currentOrder.items.length > 0 ? (
                currentOrder.items.map(({ product, quantity }) => (
                  <div key={product.id} className="flex justify-between">
                    <span>{product.name} ({product.weight} x {quantity})</span>
                    <span className="font-semibold text-[#131b2e]">₹{product.price * quantity}</span>
                  </div>
                ))
              ) : (
                <>
                  ...
                  <div className="flex justify-between"><span>Fresh Local Vegetables</span><span>₹289.50</span></div>
                </>
              )}
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-1 text-[10px] text-[#6e797a]">
            <span className="flex items-center gap-1 text-[#006a48] font-bold">
              ...
              Paid via {currentOrder?.paymentMethod || 'UPI'}
            </span>
            <button 
              onClick={(e) => {
                e.stopPropagation();
                alert('Tax Invoice downloaded for order ' + orderId);
              }}
              className="text-[#00676d] font-bold hover:underline cursor-pointer"
            >
              Download Invoice
            </button>
          </div>
        ...
        <div className="flex items-center justify-between pt-1 px-1">
          <button 
            onClick={handleSupportWhatsApp}
            className="flex items-center gap-1 text-[10px] text-[#6e797a] hover:text-[#00676d] transition-colors cursor-pointer"
          >
